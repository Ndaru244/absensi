        ...
        <tr class="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 transition">
            ...
            <td class="p-4 align-top">
                <div class="flex flex-wrap gap-2">
                    ...
                    .map(
                        (st) => `
                        <button onclick="updateStatus('${id}', '${st}')" class="px-3 py-1.5 rounded-lg text-xs font-bold border shadow-sm transition-all transform active:scale-95 flex items-center gap-1 ${s.status === st
                                ? _getColor(st)
                                : "bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 text-gray-500 hover:bg-gray-100"
                            }">
                            ${st === "Hadir"
...
window.updateStatus = (id, newStatus) => {
    ...
    ["Izin", "Alpa"].includes(newStatus)
        ? showPrompt(`Alasan ${newStatus}:`, (k) => {
            s.status = newStatus;
            s.keterangan = k || "-";
            update();
        })
        : ((s.status = newStatus), (s.keterangan = "-"), update());
};
...
window.exportToPDF = () =>
    !state.localData
        ? null
        : !state.localData.is_locked
            ? showToast("Kunci data dulu!", "warning")
            : exportToPDF(
                state.localData,
...
window.loadMonthlyReport = async () => {
    const [kls, month, tbody] = [
        ...
        (id) => document.getElementById(id).value || document.getElementById(id)
    );
    ...
    try {
        ...
        // Header
        let head =
            '<th class="p-3 sticky left-0 bg-gray-100 dark:bg-gray-700 z-30 border dark:border-gray-600 min-w-[180px]">Nama Siswa</th>';
        for (let i = 1; i <= days; i++)
            head += `<th class="p-1 text-center w-8 border dark:border-gray-600 text-[10px] text-gray-500">${i}</th>`;
        document.getElementById("headerRowBulanan").innerHTML =
            head +
            '<th class="p-2 border bg-green-100 text-green-800">H</th><th class="p-2 border bg-yellow-100 text-yellow-800">S</th><th class="p-2 border bg-blue-100 text-blue-800">I</th><th class="p-2 border bg-red-100 text-red-800">A</th>';

        // Body
        let map = {};
        Object.keys(master).forEach(
            (id) => (map[id] = Array(days + 1).fill({ status: "-", keterangan: "-" }))
        );
        reports.forEach((r) => {
            const d = parseInt(r.tanggal.split("-")[2]);
            Object.keys(r.siswa).forEach((id) => {
                if (map[id]) map[id][d] = r.siswa[id];
            });
        });
        ...
        tbody.innerHTML = `<tr><td class="p-4 text-center text-red-500">❌ ${e.message}</td></tr>`;
    }
};
